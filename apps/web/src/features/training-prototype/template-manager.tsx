// PROTOTYPE — template archive/restore/default management. Stands in for
// "Training Settings → Archived Templates" (checked: /settings already has
// a placeholder Training section waiting for exactly this). A Dialog here
// rather than wiring into the real /settings route, since this prototype
// lives on its own throwaway page. Answers ticket 04 on
...
export function TemplateManager({
  ...
  templates: Template[];
  onArchive: (id: string) => ActionResult;
                  ...
                  {active.map((tpl) => (
                    <li key={tpl.id} className="flex items-center justify-between rounded-lg border px-2.5 py-1.5">
                      ...
                      <span className="flex items-center gap-1">
                        {!tpl.isDefault ? (
                          <Button size="xs" variant="ghost" onClick={() => onSetDefault(tpl.id)}>
                            Make default
                          </Button>
                        ) : null}
                        <Button size="xs" variant="ghost" onClick={() => handleArchive(tpl.id)}>
                          Archive
                        </Button>
                      </span>
                    </li>
                    ...
                    <ul className="space-y-1">
                      {archived.map((tpl) => (
                        ...
                        >
                          ...
                          <Button size="xs" variant="ghost" onClick={() => onRestore(tpl.id)}>
                            Restore
                          </Button>
                        </li>
                      ))}
